...
having assigned themselves $s$ to the title,
...
the preliminary Steps 1-3 must take place.
...
2. Issuer $I$ privately communicates $s$ to holder $H$.
...
5. Holder $H$ stores $r$ in their local database as the private witness
to $s$.

6. Holder $H$ privately communicates $(g^r, c, s)$ to issuer $I$ with the
request to approve $c$ and append it to $L$.

7. Issuer $I$ checks their local database to see if $s$ corresponds indeed
to any title awarded to $H$. If yes, issuer $I$ proceeds to Step 8; otherwise
$H$'s request is rejected and the certification phase terminates.

8. Issuer $I$ computes $h^s$ and uses it to check if $c = g^r h^s$. If yes,
$I$ proceeds to Step 9; otherwise $H$'s request is rejected and
the certification phase terminates.

9. Issuer $I$ appends $c$ to $L$ with tag COMM and informs $H$.
...
Let a verifier $V$ meet anonymously $H$ online. $H$ wants to prove that
they possess a title (in this case: $t$) by only referring to its serial number,
(i.e., without revealing part of its content).

10. Holder $H$ detects in their local database the serial number $s$,
which identifies $t$, along with the private witness $r$ to it.

11. Holder $H$ runs the zk-SNARK Prover over $(r, s)$
to generate $\pi = \text{Prover}(\mathit{pk}, s, r)$

The produced $\pi$ is namely the zk-SNARK proof that $H$ knows $r$ such that
$Comm(r, s)$ appears in the ledger as a certified commitment. Equivalently,
$\pi$ proves possession of an awarded title on behalf of $H$.

12. Holder $H$ privately communicates $(\pi, s)$ to $V$.

13. Verifier $V$ feeds $\pi$ to the
